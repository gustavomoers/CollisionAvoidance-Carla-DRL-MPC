...
class MPC:
    ...
    def __init__(self,x=[0],y=[0],yaw=[0],v=[0],
                 ...
                 max_steering_angle = 0.61, a_max = 2, a_min = -1.5,a_rate_max=1.5, a_rate_min=-3, steer_rate=0.3,
        ...
        self.a_max = a_max
        self.a_min = a_min
        self.steer_rate_max = steer_rate
    ...
    def get_ref_traj(self):
        x = np.array([-17, -19.92, -21, -25, -10, -10, -10, -10, -1, -1])
        y =np.array([-223, -220.06, -200.059, -170.059, -160, -150, -130, - 120, -100, - 99])
    ...
    def solve(self, z_intital, z_ref):
            ...
            a,delta, st = self.linear_mpc(z_ref,z_intital, self.prev_deltas, self.dt)
            if a is None:
    ...
    def run_step(self):
        ...
        for i in range(0,self.z_ref.shape[1]-self.Hp):
            ...
            # print(self.z_ref)
            A,B,C = self.get_linearized_dynamics(self.x0[3][0],delta,self.x0[2][0],self.ts)
            z = A @ self.x0 + B @ np.array([[a],[delta]]) + C
            print(f'outputs: {z}')
            self.x0=z
